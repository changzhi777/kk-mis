...
kk-mis 是一套面向中小型企业的 **一体化管理 SaaS 系统**，通过单一平台统一管理会议纪要、企业组织、财务收支、卡券资产、代理分销与 OA 办公，消除系统割裂与数据孤岛，降低管理成本。
...
```mermaid
graph TD
    Root["(根) kk-mis · MIS 系统"] --> Apps["apps"]
    ...
    Admin -.uses.-> PG[("PostgreSQL<br/>kk_mis")]
    MN -.uses.-> PG
...
5. **admin + meeting-notes** → **PostgreSQL**（`kk_mis` 库，postgres 用户）+ **Redis**
...
- Nginx 配置：`../infra/nginx/kk-mis.conf`（会议纪要 + ASR 集群）+ `kk-mis-admin.conf`（admin）
...
### 数据库 / 缓存
- **PostgreSQL**（生产）：库名 `kk_mis`，用户 `postgres`（注意：非 `kk_mis`）
- **Redis**：admin 用 DB 1，meeting-notes 用 DB 0
- **SQLite**（开发）：`./storage/admin.db` / `./storage/kk_mis.db`
...
| `POSTGRES_DB` | `kk_mis`（meeting-notes）/ `kk_admin`（admin） | |
| `JWT_SECRET` | `kk-mis-jwt-secret-change-in-prod` | 两服务必须一致 |
| `GLM_API_KEY` | 空 | 智谱 GLM API Key |
| `MLX_ASR_API_KEY` | `kk-mis-asr-local-dev-key-2026` | meeting-notes 与 mlx-asr 必须一致 |
...
- **生产数据库用户是 `postgres`**，不是 `kk_mis`（容易踩坑）
